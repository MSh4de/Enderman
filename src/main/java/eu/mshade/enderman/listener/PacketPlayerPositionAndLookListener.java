...
public class PacketPlayerPositionAndLookListener implements EventListener<MinecraftPacketInPlayerPositionAndLook> {
    ...
    public void onEvent(MinecraftPacketInPlayerPositionAndLook event) {
        EnderFrame.get().getPacketEventBus().publishAsync(new DefaultMinecraftPacketMoveAndLookEvent(event.getSessionWrapper().getPlayer(), event.getX(), event.getY(), event.getZ(), event.getYaw(), event.getPitch(), event.isOnGround()));
